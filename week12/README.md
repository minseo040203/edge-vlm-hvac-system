...
| CI 자동 테스트 실행 | `.github/workflows/test-coverage.yml` | ✅ |
...
| 실패 시 스크린샷 저장 | `playwright.config.js`, CI Artifact | ✅ |
...
2. 메인 대시보드 제목 확인
3. Feature Flags 영역 확인
4. A/B Test Assignments 영역 확인
5. Admin 사용자 버튼 클릭
6. VLM Insight Panel 표시 확인
7. Experiment Event Log 표시 확인
```
...
| Job | 설명 |
|------|------|
| `unit-test-coverage` | Vitest 단위 테스트 및 coverage 측정 |
| `playwright-e2e` | Playwright E2E 테스트 실행 |
